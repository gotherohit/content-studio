...
For code research, Vajra also has **grep** for regular-expression search and **apply_patch** for a unified diff to one existing UTF-8 file. Grep uses bundled ripgrep; it skips protected data and dependencies and returns up to 100 bounded matches. A patch can target a research file or a `project/` file, but the full before/after is shown for one-time approval. A stale patch or a file changed while you reviewed it is refused. This is useful for scripts and notes alongside your project; do not point it at files you have not asked Vajra to edit.

### Skills and MCP tools

Open **Skills & MCP** from Vajra's sidebar, or **Settings → Vajra skills & MCP**. Choose **Current project** to save extensions in that project's `.ai` folder, or **Global** to use the app folder for all conversations. Global skills and servers are also available in a project conversation; project ones do not appear in Global research. Each skill is a `SKILL.md` file with a lowercase id and instructions. An optional YAML `description:` helps Vajra decide when to call `read_skill`; it can also use `list_skills` and `read_skill_resource` to inspect supporting text files you put beside that skill. The settings editor changes only `SKILL.md`, and removing a skill leaves other files in its folder alone.

An MCP server can be a **local stdio** command with arguments or a **Streamable HTTP** endpoint. Save it, then press **Test connection** to see the discovered tools. Enabled servers connect when a Vajra run starts; a failed server does not stop the other tools. Every MCP tool invocation pauses for **Allow once** or **Decline** with the server, tool and arguments visible. The connection closes when the run ends or is stopped. A local stdio server is a program running under your account; only add commands you trust. Imported servers or commands changed outside Settings show **Review** and will not connect until you select and save them. HTTP redirects are refused. For credentials, put a token in your system environment and use `${NAME}` in stdio environment settings or `Bearer ${NAME}` in an HTTP header so the token is not saved in MCP settings. The UI supports up to eight servers per scope; each server contributes up to 40 tools. External tool descriptions and results are treated as untrusted data.
...
**Stop** cancels a run. Closing its Vajra pane also cancels it, keeping completed tool results and any partial text already received. Runs are limited to 12 model steps and 15 minutes. After stopping, interruption, failure or the step limit, **Continue** starts a new turn that asks Vajra to inspect saved progress before repeating actions; it still asks for write, shell and MCP approvals. You can also send a more specific follow-up. Older complete turns leave the model's working context, but remain on disk and can be read by the history tool. A provider/model without tool support will report an error; protocol compatibility alone does not guarantee tool support.
